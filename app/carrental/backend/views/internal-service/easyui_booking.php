...
$urlsArray = [
    'url' => \yii\helpers\Url::to(['internal-service/applying_list', $filterModel->fieldName('type')=> backend\models\Pro_inner_applying::TYPE_VEHICLE_INNER_USE]),
];

$toolbarArray = [
    Yii::$app->user->can('internal-service/usevehicle_add') ? CMyHtml::formatDatagridToolConfig(CMyHtml::DG_TOOL_APPEND, '', ['dialog'=>\yii\helpers\Url::to(['internal-service/usevehicle_add'])]) : null,
    Yii::$app->user->can('internal-service/usevehicle_edit') ? CMyHtml::formatDatagridToolConfig(CMyHtml::DG_TOOL_EDIT, '', ['dialog'=>\yii\helpers\Url::to(['internal-service/usevehicle_edit']), 'needSelect' => true]) : null,
    Yii::$app->user->can('internal-service/usevehicle_approval') ? CMyHtml::formatDatagridToolConfig(CMyHtml::DG_TOOL_BUTTON, \Yii::t('locale', 'Approval'), ['dialog'=>\yii\helpers\Url::to(['internal-service/usevehicle_approval']) , 'needSelect' => true], 'icon-ok') : null,
    Yii::$app->user->can('internal-service/usevehicle_delete') ? CMyHtml::formatDatagridToolConfig(CMyHtml::DG_TOOL_REMOVE, '', ['ajax'=>\yii\helpers\Url::to(['internal-service/usevehicle_delete']), 'needSelect' => true]) : null,
