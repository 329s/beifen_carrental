...
$urlInfo = ['order/order_with_vehicle_list'];
...
if ($status == \common\models\Pro_vehicle_order::STATUS_WAITING) {
    ...
    $columnFields[] = 'operation_waiting';
    $sortName = 'start_time';
    $sortOrder = 'asc';
}
    ...
    $columnFields[] = 'operation_violation';
    $dlgWidth = '1006px';
    $dlgHeight = '520px';
    $sortName = 'new_end_time';
    $sortOrder = 'asc';
    ...
    $columnFields[] = 'operation_complete';
    $sortName = 'new_end_time';
    $sortOrder = 'desc';
}
...
if ($status == \common\models\Pro_vehicle_order::STATUS_COMPLETED
    || $status == \common\models\Pro_vehicle_order::STATUS_RENTING) {
    if (false) {
        $exportUrl = \yii\helpers\Url::to(['order/export_order_data', 'status'=>$status]);
    ...
    elseif (Yii::$app->user->can('statement/orderbymonthly')) {
        $exportUrl = \yii\helpers\Url::to(['statement/orderbymonthly', 'status'=>$status]);
