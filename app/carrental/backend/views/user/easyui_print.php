...
$reloadUrl = \yii\helpers\Url::to(['user/print', 'status'=>0]);

$dataProvider = new yii\data\ArrayDataProvider([
    'allModels' => $models,
    'modelClass' => '\common\models\Pro_sign_up',
    'pagination' => [
...
// export button
if (\Yii::$app->user->can('user/print-export')) {
    $exportUrl = \yii\helpers\Url::to(['user/print-export', 'export'=>'excel',  'date'=>$date, 'date_start'=>$date_start]);
    $headerHtmlArray[] = CMyHtml::tag('a', '导出', ['class'=>'btn btn-info', 'href'=>$exportUrl]);
}
//$headerHtmlArray[] = \yii\helpers\Html::endTag('div');

$arrScripts[] = "var dailyQueryParams{$autoId} = { date_start:'{$date_start}', date:'{$date}'};\n".
