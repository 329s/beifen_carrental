...
function funcGenerateVehicleInfoCell(row) {
    ...
    var nextUpkeepTime = parseInt(row.next_upkeep_time);
    var upkeepTime = nextUpkeepTime - parseInt(row.last_upkeep_time);
    var renewalTimeTCI = parseInt(row.tci_renewal_time);
    var renewalTimeVCI = parseInt(row.vci_renewal_time);
    var annualInspectionTime = parseInt(row.annual_inspection_time);
    ...
    if (row.status == <?= \common\models\Pro_vehicle::STATUS_BOOKED ?>) {
        arrStatusInfo.push('<font size="12" color="green">'+'<?= Yii::t('carrental', 'Booked: ') ?> '+$.custom.utils.formatTime('yyyy-MM-dd hh:mm', row.rent_start_time)+'—'+$.custom.utils.formatTime('yyyy-MM-dd hh:mm', row.rent_end_time)+'</font>');
    }
    else if (row.status == <?= \common\models\Pro_vehicle::STATUS_RENTED ?>) {
        arrStatusInfo.push('<font size="12" color="green">'+'<?= Yii::t('carrental', 'Rented: ') ?> '+$.custom.utils.formatTime('yyyy-MM-dd hh:mm', row.rent_start_time)+'—'+$.custom.utils.formatTime('yyyy-MM-dd hh:mm', row.rent_end_time)+'</font>');
    }
    ...
    if (nextUpkeepTime > 0 && nextUpkeepTime < curTim + 30*86400 && upkeepTime > 1000) {
        var leftDays = Math.floor((parseInt(row.next_upkeep_time) - curTim) / 86400);
    ...
    if (row.stop_office_id != row.belong_office_id) {
        arrStatusInfo.push('<font size="12" color="red">'+'<?= Yii::t('carrental', 'Stop: ') ?> '+row.stop_office_disp+'</font>');
    }
    ...
    arrCellHtmls.push('<p class="f-toe"><font color="gray">'+row.vehicle_model+'</font></p>');
    arrCellHtmls.push('<div class="u-img2"><span style="text-align:center;"><img src="'+row.vehicle_image+'" alt="'+row.vehicle_model+'" style="clear:both; display:block; margin:auto;" /></span></div>');
    arrCellHtmls.push('<h3><a href="javascript:void(0)">'+row.plate_number+'</a></h3>');
    arrCellHtmls.push('<p class="f-toe"><font color="orange" style="font-weight:bold">'+row.belong_office_disp+'</font></p>');
...
function funcOnVehicleListLoaded<?= $autoId ?>(data) {
    ...
    for (var i in obj.rows) {
        ...
        arrHtmls.push('<li onclick="easyuiFuncNavTabAddDoNotKnownId(\'<?= Yii::t('carrental', 'Book vehicle') ?>\', \'<?= \yii\helpers\Url::to(['vehicle/rent_register', 'vehicle_id'=>'']) ?>'+row.id+'\', undefined, false);">');
