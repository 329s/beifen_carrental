...
$arrData = [
	...
	[
		[$objInquiry->getAttributeLabel('money'), $objInquiry->money],
		[$objInquiry->getAttributeLabel('fen'), $objInquiry->fen],
	],
	[
		[$objOrder->getAttributeLabel('customer_name'), $objOrder->customer_name],
...
if ($objOrder) {
    $arrData[] = [
		[$objOrder->getAttributeLabel('customer_telephone'),$objOrder->customer_telephone],
		[$objInquiry->getAttributeLabel('date'), date('Y-m-d H:i:s', $objInquiry->date_time)],
    ];
	$arrData[] = [
...
$arrData[] = [
	[$objInquiry->getAttributeLabel('area'), $objInquiry->area],
	[$objInquiry->getAttributeLabel('act'), $objInquiry->act],
];
...
setTimeout(function() {
    easyuiFuncDatagridSetOptionsCustomValue('#{$dgId}', 'queryParams', {vehicle_id:{$vehicleId}, order_id:{$orderId},inquiryId:{$inquiryId}, '{$yiiCsrfKey}':'{$yiiCsrfToken}'});
    easyuiFuncDatagridSetOptionsCustomValue('#{$dgId}', 'modelName', '{$objForm->formName()}');
    easyuiFuncDatagridSetOptionsCustomValue('#{$dgId}', 'defaultValues', {vehicle_id:{$vehicleId}, order_id:{$orderId},inquiryId:{$inquiryId}, status:{$violationDefaultStatus}});
