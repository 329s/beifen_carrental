...
$inputs = [
    ...
    ['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $objForm->fieldName('last_upkeep_mileage'),
        'label' => $objData->getAttributeLabel('last_upkeep_mileage'),
        'value' => $objData->last_upkeep_mileage,
        'htmlOptions' => ['style'=>"width:200px",
            'id' => "{$idPrefix}last_upkeep_mileage{$autoId}",
    ...
    ['type' => CMyHtml::INPUT_DATEBOX, 'name' => $objForm->fieldName('last_upkeep_time'),
        'label' => $objData->getAttributeLabel('last_upkeep_time'),
        'value' => (empty($objData->last_upkeep_time) ? '' : date('Y-m-d', $objData->last_upkeep_time)),
        'htmlOptions' => ['style'=>"width:200px", 'editable'=>false,
            'onChange'=>"updateNextUpkeepTime{$autoId}",
            'id' => "{$idPrefix}last_upkeep_time{$autoId}",
            ],
        'columnindex' => 0,
    ],
...
function updateNextUpkeepTime{$autoId}() {
    var lastTime = $.custom.utils.toTimestamp($('#{$idPrefix}last_upkeep_time{$autoId}').datebox('getValue'));
    var interval = parseInt($('#{$idPrefix}upkeep_time_interval{$autoId}').numberbox('getValue'));
    if (isNaN(interval)) { interval = 0; }
    
    var nextTime = lastTime + (interval * 86400*30);
