 ...
 */
class OrderhourEditorEasyuiWidget extends \yii\base\Widget
{
    /**
    ...
    public function init()
    {
        ...
        $this->_urlGetPrice = \yii\helpers\Url::to(['orderhour/get_order_price', '_'=>$curTime]);
        $this->_urlGetPreferentialData = \yii\helpers\Url::to(['options/preferential_combo_data']);
        $this->_urlGetUserOrders = \yii\helpers\Url::to(['orderhour/userrentlist_index', '_'=>$curTime]);
        ...
        $this->_urlPaymentinput = \yii\helpers\Url::to(['orderhour/paymentinput', 'is_relet'=>0, 'is_settlement'=>$this->_enableSettlement?1:0, 'order_action'=> $this->action]).'&order_id=';
		...
		if($this->_orderModel->flag > 0){
			$this->_oneWayTimePrice = \common\models\Pro_vehicle_fee_plan::getOneWayTimePrice($this->_orderModel->flag);
		}
    ...
    public function run()
    {
        ...
        $htmlArray[] = \yii\helpers\Html::jsFile("{$this->_rootUrl}app/carrental/backend/web/js/orderhour.js");
        ...
        $htmlArray[] = CMyHtml::form($this->title, \yii\helpers\Url::to(['orderhour/edit']), 'post', $formHtmlOptions, $inputs, $buttons, $hiddenFields);
        
        // header html
        $htmlArray[] = CMyHtml::beginTag('div', ['id'=>$headerId, 'style'=>"text-align:right"]);
        
        //$paymentInputUrl = \yii\helpers\Url::to(['orderhour/paymentinput', 'order_id'=>empty($this->_orderModel->id) ? '0' : $this->_orderModel->id]);
    ...
    protected function getVehicleInputFieldsInfo() {
        ...
        $_days = [1,2,3,4,5,6,7,8,9];
        foreach ($_days as $day) {
            $classicalRentDaysButtons[] = \yii\bootstrap\Html::button(\Yii::t('locale', '{number} hours', ['number'=>$day]), ['onclick'=>"funcSetClassicalRentDays{$this->_autoId}({$day})", 'class'=>'btn btn-default']);
        }
        ...
        $inputs = [
            ...
            ['type' => CMyHtml::INPUT_TYPE_HTML, 'label'=> \Yii::t('carrental', 'Recommend vehicle rent days:'),'html' => implode("\n", $classicalRentDaysButtons)],
            ['type' => CMyHtml::INPUT_TYPE_SUBGROUP],
            ['type' => CMyHtml::INPUT_DATETIMEBOX, 'name' => $this->_formModel->fieldName('start_time'),
                ...
                'htmlOptions' => array_merge($orderDefaultOptions, ['required' => true, 'editable'=>false, 'id'=>"{$this->_idPrefix}start_time{$this->_autoId}",
            ...
            ['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('rent_days'),
                'label' => $this->_orderModel->getAttributeLabel('rent_hour'),
            ...
            ['type' => CMyHtml::INPUT_COMBOBOX, 'name' => $this->_formModel->fieldName('pay_type'),
                ...
                'data' => \common\components\OrderModule::getHourPriceTypeArray(),
            ...
            ['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('rent_per_day'),
                'label' => $this->_orderModel->getAttributeLabel('rent_per_hour'),
			...
			['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('price_address_km'),
                'label' => $this->_orderModel->getAttributeLabel('price_address_km'),
                'value' => $this->_orderModel->price_address_km,
				'htmlOptions' => array_merge($orderDefaultOptions,['required' => false, 'editable'=>false,'precision'=>2,'id'=>"{$this->_idPrefix}price_address_km{$this->_autoId}", 'tailhtml'=>\Yii::t('locale', 'CNY')]),
            ],
			
			['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('address_km'),
                'label' => $this->_orderModel->getAttributeLabel('address_km'),
                'value' => $this->_orderModel->address_km,
            ...
            ['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('price_deposit_violation'),
                ...
                'value' => $this->_oneWayTimePrice['oneWayDeposit'],
                'htmlOptions' => array_merge($orderDefaultOptions, ['required' => true, 'editable'=>false, 
    ...
    protected function getSettlementInputFieldsInfo() {
        ...
        $arrMainInfo = [
            ...
            CMyHtml::tag('font', \Yii::t('locale', '{number} hours', ['number'=>$this->_orderModel->rent_days]), ['style'=>'color:red']),
            CMyHtml::tag('font', "]", ['style'=>'color:green'])
        ];
        ...
        $inputs = [
			...
			['type' => CMyHtml::INPUT_NUMBERBOX, 'name' => $this->_formModel->fieldName('price_working_loss'),
				'label' => $this->_orderModel->getAttributeLabel('price_working_loss'),
				'value' => $this->_orderModel->price_working_loss,
				'htmlOptions' => array_merge($orderDefaultOptions, ['required' => false, 
					'id'=>"{$this->_idPrefix}settlement_price_other{$this->_autoId}",
						'onChange'=>"funcUpdateTotalPrice{$this->_autoId}",
					'tailhtml'=>\Yii::t('locale', 'CNY'),
...
function funcOnSelectVehicle{$this->_autoId}(idx, row, skipUpdateRentDays) {
    ...
    if ({$this->vehicleId} != row.id && row && parentForm.length == 1) {
        ...
        // $('#{$this->_idPrefix}price_poundage{$this->_autoId}', parentForm).textbox('setValue', 0);
        // $('#{$this->_idPrefix}unit_price_basic_insurance{$this->_autoId}', parentForm).textbox('setValue', row.vehicle_basic_insurance);
        $('#{$this->_idPrefix}unit_price_basic_insurance{$this->_autoId}', parentForm).textbox('setValue', 0);
        // $('#{$this->_idPrefix}price_deposit_violation{$this->_autoId}', parentForm).textbox('setValue', row.vehicle_rent_deposit);
        $('#{$this->_idPrefix}price_deposit_violation{$this->_autoId}', parentForm).textbox('setValue', {$this->_oneWayTimePrice['oneWayDeposit']});
...
function funcCalculateRentDaysByTime{$this->_autoId}(startTime, endTime) {
    ...
    var extraDuration = rentDuration % 3600;
    var rentDays = Math.floor(rentDuration / 3600);
    var rentHours = 0;
    if (extraDuration >= 3600) {
        rentDays++;
    }
...
function funcGetOrderTimes{$this->_autoId}(forceReturnFixedTime) {
    ...
    if ({$jsIsSettlement}) {
        ...
        settlement_car_returned = $.custom.utils.toTimestamp($('#{$this->_idPrefix}settlement_car_returned_at{$this->_autoId}', parentForm).datetimebox('getValue'));
		var rentMinute = settlement_car_returned - endTime;
		var rentDaysData = funcCalculateRentDaysByTime{$this->_autoId}(startTime, endTime);
		return {startTime:startTime, endTime:endTime, rentDays:rentDaysData.rentDays, rentHours:rentDaysData.rentHours,rentMinute:rentMinute};
    }
    else {
        if (startTime <= 0) {
		...
		var rentDaysData = funcCalculateRentDaysByTime{$this->_autoId}(startTime, endTime);
		return {startTime:startTime, endTime:endTime, rentDays:rentDaysData.rentDays, rentHours:rentDaysData.rentHours,rentMinute:0};
    }
...
function funcOnSelectVehicleModel{$this->_autoId}(record) {
    ...
    if (record) {
        ...
        var setPrices = {
            ...
            '#{$this->_idPrefix}price_deposit_violation{$this->_autoId}':{$this->_oneWayTimePrice['oneWayDeposit']},
...
function funcOnStartTimeChanged{$this->_autoId}(newValue,oldValue) {
    ...
    if (opts.__settingTime === undefined && opts2.__settingTime === undefined) {
        ...
        if (startTime > 0 && endTime > 0 && ((endTime - startTime) % 3600 > 9)) {
            var rentDaysData = funcCalculateRentDaysByTime{$this->_autoId}(startTime, endTime);
            ...
            endTime = startTime + 3600 * rentDays;
...
function funcSetClassicalRentDays{$this->_autoId}(days) {
    ...
    timeData.endTime = timeData.startTime + (3600 * days);
...
function funcUpdateRentPrice{$this->_autoId}(vehicleModelId, timeData) {
    ...
    easyuiFuncAjaxSendDataWithoutAlert('{$this->_urlGetPrice}', 'get', getPriceParams, function(data){
        ...
        if (obj.code == 0) {
            ...
            obj.price_address_km = parseFloat($('#{$this->_idPrefix}price_address_km{$this->_autoId}', parentForm).numberbox('getValue'));
            obj.price_take_car = parseFloat($('#{$this->_idPrefix}price_take_car{$this->_autoId}', parentForm).numberbox('getValue'));
            obj.price_return_car = parseFloat($('#{$this->_idPrefix}price_return_car{$this->_autoId}', parentForm).numberbox('getValue'));
            obj.price_overtime = parseFloat($('#{$this->_idPrefix}settlement_price_overtime{$this->_autoId}', parentForm).numberbox('getValue'))
...
function funcUpdateTotalPrice{$this->_autoId}() {
    ...
    price_address_km = parseFloat($('#{$this->_idPrefix}price_address_km{$this->_autoId}', parentForm).numberbox('getValue'));
    ...
    priceOvertime = CalculatingTheTimeoutPrice(timeData.rentMinute);
    ...
    var priceTotal = priceRent + priceOptional + pricePoundage + priceDesignatedDriving + priceBasicInsurance +
        ...
        - (pricePreferential + priceFree) + price_address_km;
...
//计算单程超时价格
function CalculatingTheTimeoutPrice(rentMinute){
	if(rentMinute > 0){
		var parentForm = $('#{$this->_formId}');
		
		var oneWayOverTimeMinutePrice =  parseInt({$this->_oneWayTimePrice['oneWayOverTimeMinutePrice']});
		var oneWayOverTimeHoursPrice =  parseInt({$this->_oneWayTimePrice['oneWayOverTimeHoursPrice']});
		var oneWayOilByKm = parseFloat({$this->_oneWayTimePrice['oneWayOilByKm']});
		
		var targetPricePerDay =  parseFloat($('#{$this->_idPrefix}rent_per_day{$this->_autoId}', parentForm).numberbox('getValue'));
		
		
		if(rentMinute <= 600){
			return 0;
		}else if(rentMinute >600 && rentMinute<= 1800){
			return targetPricePerDay + oneWayOverTimeMinutePrice * oneWayOilByKm;
		}else if(rentMinute >1800 && rentMinute<= 3600){
			return targetPricePerDay + oneWayOverTimeHoursPrice * oneWayOilByKm;
		}else if(rentMinute > 3600){
			return targetPricePerDay + oneWayOverTimeHoursPrice * oneWayOilByKm * Math.ceil(rentMinute / 3600);
		}
	}else{
		return 0;
	}
...
EOD;
        ...
        if (!empty($this->_orderModel->id)) {
            $getPaidAmountUrl = \yii\helpers\Url::to(['orderhour/get_order_paid_amount', 'order_id'=>$this->_orderModel->id]);
            $arrScripts[] = <<<EOD
