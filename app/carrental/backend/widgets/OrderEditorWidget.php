 ...
 */
class OrderEditorWidget extends \yii\base\Widget
{
    /**
    ...
    public function init()
    {
        ...
        if ($this->action == 'settlement' || $this->_orderModel->status >= \common\models\Pro_vehicle_order::STATUS_VIOLATION_CHECKING) {
    ...
    public function run()
    {
        ...
        if (!$this->_isAdmin) {
            $hiddenFields[$this->_formModel->fieldName('belong_office_id')] = ['id'=>"{$this->_idPrefix}belong_office_id_hidden{$this->_autoId}", 'value'=>intval($this->_orderModel->belong_office_id)];
        }
    ...
    protected function getCustomerInputFieldsInfo() {
        ...
        $inputs = [
            ...
            ['type' => CMyHtml::INPUT_TEXTBOX, 'name' => $this->_formModel->fieldName('customer_id'),
                ...
                'htmlOptions' => array_merge($customerDefaultOptions, ['required' => true, 
                    'id'=>"{$this->_idPrefix}customer_id{$this->_autoId}"]),
                'columnindex' => 1,
            ],
    ...
    protected function getVehicleInputFieldsInfo() {
        ...
        if ($this->_isAdmin) {
            $inputs[] = ['type' => CMyHtml::INPUT_COMBOTREE, 'name' => $this->_formModel->fieldName('belong_office_id'),
                ...
                'data' => \common\components\OfficeModule::getOfficeComboTreeData(),
                'htmlOptions' => array_merge($orderDefaultOptions, ['required' => true, 'editable'=>false, 
                    'id' => "{$this->_idPrefix}belong_office_id{$this->_autoId}"]),
                'columnindex' => 0,
            ];
        }
...
function funcOnSelectVehicle{$this->_autoId}(idx, row, skipUpdateRentDays) {
    ...
    if ({$this->vehicleId} != row.id && row && parentForm.length == 1) {
        ...
        $('#{$this->_idPrefix}vehicle_color{$this->_autoId}', parentForm).combobox('setValue', row.color);
        ...
        if ($('#{$this->_idPrefix}office_id_rent{$this->_autoId}', parentForm).combotree('getValue') == 0) {
            $('#{$this->_idPrefix}office_id_rent{$this->_autoId}', parentForm).combotree('setValue', row.stop_office_id);
        }
        if ($('#{$this->_idPrefix}office_id_return{$this->_autoId}', parentForm).combotree('getValue') == 0) {
            $('#{$this->_idPrefix}office_id_return{$this->_autoId}', parentForm).combotree('setValue', row.stop_office_id);
        }
        ...
        if (!{$belongOfficeId} && row.stop_office_id > 0) {
            if (oBelongOfficeId && oBelongOfficeId.length == 1) {
                oBelongOfficeId.combotree('setValue', row.stop_office_id);
            } else if (oBelongOfficeId2 && oBelongOfficeId2.length == 1) {
                oBelongOfficeId2.val(row.stop_office_id);
            }
        }
...
function funcOnselectUser{$this->_autoId}(record) {
    ...
    if (record.driver_license_time > 0) {
        if ($('#{$this->_idPrefix}customer_driver_license_time{$this->_autoId}', parentForm).length > 0) {
            $('#{$this->_idPrefix}customer_driver_license_time{$this->_autoId}', parentForm).textbox('setValue', $.custom.utils.humanTime(record.driver_license_time));
        }
    }
...
function funcSearchUserOrders{$this->_autoId}() {
    ...
    var userId = $('#{$this->_idPrefix}user_id{$this->_autoId}', parentForm).val();
    var userName = $('#{$this->_idPrefix}customer_name{$this->_autoId}', parentForm).textbox('getValue');
    if (userId && userName) {
