...
class VehicleService extends BaseService
{
    ...
    public static function getMonthlyVehicleChangeDataColumns($status,$pay_type = 0)
    {
        $cols = [
            'belong_office_id' => ['attribute'=>'belong_office_id','header'=>'车辆所属门店','value'=>function($model){ return $model['belong_office_disp']; }], 
            'stop_office_id' => ['attribute'=>'stop_office_id','header'=>'车辆停靠门店','value'=>function($model){ return $model['stop_office_disp']; }], 
            'vehicle_id' => ['attribute'=>'vehicle_id','value'=>function($model){ return $model['plate_number']; }], 
            'engine_number' => ['attribute'=>'engine_number','header'=>'发动机号','value'=>function($model){ return $model['engine_number']; }], 
            'vehicle_number' => ['attribute'=>'vehicle_number','header'=>'车架号','value'=>function($model){ return $model['vehicle_number']; }], 
            'status' => ['attribute'=>'status','header'=>'车辆状态','value'=>function($model){ return $model['status']; }], 
            'operation' => ['attribute'=>'engine_number','header'=>'操作','value'=>function($model){ return $model['engine_number']; }], 
        ...
        $columns = [$cols['vehicle_id'], $cols['vehicle_model_id'], 
            $cols['belong_office_id'],$cols['stop_office_id'],$cols['engine_number'],$cols['vehicle_number'],$cols['status'],$cols['operation']
        ];

        return $columns;
    }

    public function getMonthlyVehicleChangeDataNew($value='')
    {
        if (empty($monthDate)) {
