 ...
 * @author kevin
 */
class OrderController  extends \backend\components\AuthorityController
{
    private $pageSize = 20;
    ...
    public function actionCancel() {
        ...
        if ($objOrder->status >= \common\models\Pro_vehicle_order::STATUS_RENTING) {
			...
			\common\components\SmsComponent::send(
				...
				\common\components\Consts::KEY_SMS_ORDER_CANCELED, 
				[
            ...
            if ($type == 'vehicle_id') {
                ...
                */
                ...
                if (abs($startTime - $objOrder->start_time)>60 || empty($objOrder->vehicle_id)) {//两个时间本相同的时候由于可能日期选择中少了秒数而导致两个时间相差
                    ...
                    $arrResult['unit_price_basic_insurance'] = $arrDeltaData['unit_price_basic_insurance'];//基本保险费
    ...
    public function actionPurchase_order_list() {
        ...
        if (!empty($serial)) {
            $objOrderId = \common\models\Pro_vehicle_order::find()->select(['id'])->where(['serial'=>$serial])->asArray()->one();
            $cdb->andWhere(['bind_id'=>$objOrderId['id']]);
        }
    ...
    {
        // \moonland\phpexcel\
        // return \dosamigos\qrcode\QrCode::png('http://www.yii-china.com');
        // return \Da\QrCode\QrCode::png('http://www.yii-china.com');
        return QrCode::png('http://www.yii-china.com');    //调用二维码生成方法
    }

    public function actionDemo()
    {
        require_once dirname(dirname(dirname(dirname(__DIR__)))).'\vendor\phpqrcode\qrlib.php';
        // return \QRcode::png('http://www.yii-china.com');
        // return $this->render('index');
        $value = 'www.yii-china.com';                  //二维码内容
        return \QRcode::png($value);
        // $errorCorrectionLevel = 'L';    //容错级别
        // $matrixPointSize = 5;           //生成图片大小

        // //生成二维码图片
        // $filename = dirname(dirname(dirname(dirname(__DIR__)))).'/public/qrcode/'.microtime().'.png';
        ...
        // //输出图片
        // imagepng($QR, dirname(dirname(dirname(dirname(__DIR__)))).'/public/qrcode/qrcode.png');
        // imagedestroy($QR);
        // return '<img src="www.ykzc_beifen.com/public/qrcode/qrcode.png" alt="使用微信扫描支付">';
    }

    public function actionDemo1(){
        echo $this->actionDemo();
    }
